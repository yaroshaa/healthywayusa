<h2 class="uppercase inline-block lg:text-3xl lt:text-2xl md:text-2xl font-medium text-dark-green xl:my-8 lg:my-8 md:my-5 sm:my-3 lt:my-3 ">
    <div class="flex justify-items-stretch items-end relative">
        <div class="wrapper-line-before-hexagon relative h-10 w-full flex flex-col items-end">
            <div class="line-before-hexagon-small h-0 w-2/5 bg-dark-green mt-4 -mb-2 mr-2 z-10 border-l-transparent border-l-4 border-dark-green border-t-10 " >

            </div>
            <div class="line-before-hexagon h-5 w-full bg-light-green mb-1.5 mr-1 " >
            </div>
        </div>
        <div class="hexagon-wrapper flex text-align -mr-1">
            <div class="m-auto ">
                <svg
                    width="40"
                    height="40"
                    viewBox="0 0 200 200"
                    id="svg1"
                    xmlns="http://www.w3.org/2000/svg"
                    xmlns:svg="http://www.w3.org/2000/svg">
                    <defs
                        id="defs1" />
                    <g
                        id="layer1">
                        <path
                            style="fill:#b0d391;stroke-width:0.264583"
                            id="path1"
                            d="m 59.996889,78.47541 -39.633628,-2e-6 L 0.54644775,44.151678 20.363264,9.8279496 l 39.633629,1.9e-6 19.816812,34.3237305 z"
                            transform="matrix(2.5040449,0,0,2.492038,-0.16123137,-6.7585058)" />
                    </g>
                </svg>
            </div>
        </div>
    </div>
    {{ $slot }}
</h2>
